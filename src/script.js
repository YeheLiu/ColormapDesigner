...
window.addEventListener('DOMContentLoaded', () => {
    ...
    colorForm.dispatchEvent(new Event('submit', { cancelable: true }));
});
...
outputStyleRadios.forEach(radio => {
    radio.addEventListener('change', () => {
        document.getElementById("colorForm").dispatchEvent(new Event('submit', { cancelable: true }));
    });
});
...
function onClick() {
    ...
    for (let i = 0; i < numSteps; i++) {
        const color = interpolateColors(color1, color2, i / (numSteps - 1), colorSpace);
        colors.push(color);
...
function interpolateColors(color1, color2, factor, colorSpace) {
    ...
    switch (colorSpace) {
        case "hsv":
            c1 = rgbToHsv(color1);
            c2 = rgbToHsv(color2);
            interpolated = interpolateValues(c1, c2, factor);
...
function interpolateValues(values1, values2, factor) {
